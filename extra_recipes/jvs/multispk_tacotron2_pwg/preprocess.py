...
def get_parser():
    parser = argparse.ArgumentParser(
        description="Preprocess for multi-speaker Tacotron",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("utt_list", type=str, help="utternace list")
    parser.add_argument("spk_id", type=int, help="speaker ID")
...
def preprocess(
    ...
    np.save(
        in_dir / f"{utt_id}-spk.npy",
        np.array([spk_id], dtype=np.int64),
        allow_pickle=False,
    )
