...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    for s in ${datasets[@]}; do
        xrun python preprocess_acoustic.py data/$s.list $db_root/wav/ $db_root/lab/ \
...
if [ ${stage} -le 98 ] && [ ${stop_stage} -ge 98 ]; then
    ...
    for model in $duration_model $acoustic_model; do
        rsync -avr $expdir/$model tmp/exp/$expname/ --exclude "epoch*.pth"
    done
    rsync -avr $expdir/synthesis_${duration_model}_${acoustic_model} tmp/exp/$expname/ --exclude "epoch*.pth"
    cd tmp
    tar czvf dnntts_exp.tar.gz exp/
    mv dnntts_exp.tar.gz ..
    cd -
    rm -rf tmp
    echo "Please check dnntts_exp.tar.gz"
fi
